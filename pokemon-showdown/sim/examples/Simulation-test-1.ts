...
export class HeuristicsPlayerAI extends RandomPlayerAI {
	...
	protected _should_switch_out(request) {
		...
		if (availableSwitches.filter(m => this._estimateMatchup(request) > 0).length) {
	...
	protected chooseMove(request, active, moves: {choice: string, move}[], canDynamax, possibleMoves): [string, boolean] {
		...
		|| (request.side.pokemon.filter((m) => Number(this._getHpFraction(m.condition)) != 0).length == 1 && mon.currentHpPercent == 1)) {
			...
			// Weather setup moves
			for (var move of possibleMoves) {
				if ((move.id || move.move) in this.WEATHER_SETUP_MOVES && currentWeather != this.WEATHER_SETUP_MOVES[(move.id || move.move)]) {
			...
			// Status Inflicting Moves
				// TODO out of scope for project but would be good to account for all abilities which make mons immune to certain status conditions
				...
				&& opponent.currentHpPercent > 0.6 && mon.currentHpPercent > 0.5) {
					...
					switch (cond) {
						...
						case "sleep":
							...
							break;
						case "confusion":
							// TODO implement confusion, too much complexity for project due to terrains, substitutes, and own tempo causing immunity
							break;
						case "poison":
	...
	protected _getPokemonPos(request, bestMatchup) {
		for (const pokemon of request.side.pokemon) {
			if (pokemon.details == bestMatchup.details) {
			}
		}
		return (request.side.pokemon).filter((pokemon) => (pokemon.details == bestMatchup.details))[0].position+1
	}
