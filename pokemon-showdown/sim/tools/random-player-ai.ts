...
export class RandomPlayerAI extends BattlePlayer {
	...
	receiveRequest(request: AnyObject) {
		if (request.wait) {
			...
			const choices = request.active.map((active: AnyObject, i: number) => {
				...
				const change = (canMegaEvo || canUltraBurst || shouldDynamax) && this.prng.next() < this.mega;
				...
				if (switches.length && (!moves.length || this.prng.next() > this.move)) {
					...
					const move_canDynamax = this.chooseMove(request, active, moves, canDynamax, possibleMoves);
					if (move_canDynamax[0] === 'undefined') {
						return 'pass'
					}
					else if (move_canDynamax[0].startsWith("switch ")) {
						return move_canDynamax[0]
					}
					const move = move_canDynamax[0]

					if (move_canDynamax[1] == true) canDynamax = true
	...
	protected chooseMove(request, active: AnyObject, moves: {choice: string, move: AnyObject}[], canDynamax: boolean, possibleMoves): [string, boolean] {
		return [this.prng.sample(moves).choice, canDynamax]
	}
